...
class FocusPointImage extends DataExtension {
	...
	public function calculateCrop($width, $height) {
		...
		else if ($cropData['x']['ScaleRatio'] > $cropData['y']['ScaleRatio']) {
		...
		if ($cropAxis) {
			...
			if ($remainder < $croppedRemainder) $focusShift-= $croppedRemainder - $remainder;
			if ($focusShift < 0) $focusShift =0;
	...
	public function CroppedFocusedImage($width,$height,$upscale=true) {
		...
		else if ($cropData = $this->calculateCrop($width, $height)) {
